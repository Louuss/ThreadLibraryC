...
int main(int argc, char *argv[])
{
  ...
  for(i=0; i<NB_MUTEX; i++) {
      if ( counter[i] == ( (int)nb * 1000 ) ) {
	  printf("La somme %lu a été correctement calculée: %lu * 1000 = %d\n", i, nb, counter[i]);
      }
      else {
	  printf("Le résultat %lu est INCORRECT: %lu * 1000 != %d\n", i, nb, counter[i]);
      }
  }
  return 0;
}
